...
function vlessUriToClashProxy(vlessUrl, name) {
...
export function generateYaml(template, vlessUrl, clientName) {
...
export function getTemplate() {
  const row = queryOne("SELECT value FROM settings WHERE key = 'clash_template'");
...
export function getVpsHost() {
  const row = queryOne("SELECT value FROM settings WHERE key = 'vps_host'");
  return row ? row.value : '';
}

export function saveVpsHost(host) {
...
export function createSubscription({ clientId, clientName, serverHost, vlessUrl }) {
...
export function regenerateAllSubscriptions() {
  const template = getTemplate();
  const subs = query('SELECT * FROM subscriptions');
  let updated = 0;
...
export function getSubscriptionBySlug(slug) {
  return queryOne('SELECT * FROM subscriptions WHERE slug = ?', [slug]);
}
...
export function deleteSubscription(clientId) {
