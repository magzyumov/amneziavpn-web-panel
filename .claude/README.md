...
│   ├── api-contract-checker.md # сверка ответов роутов с ожиданиями фронта
│   └── docs-checker.md         # сверка README с кодом
...
### `api-contract-checker` — контракт backend↔frontend

Читает `res.json` в роутах и сверяет с типами в `api.ts` и разбором в страницах.
Нужен потому, что этот контракт компилятор не проверяет: `api.get<T>()` — просто
утверждение разработчика. Запускать после правки роута или `api.ts`.

### `docs-checker` — дрейф README

Сверяет README с кодом по четырём осям: протоколы, переменные окружения,
эндпоинты, дерево модулей и факты в прозе. Запускать после заметных изменений и
перед публичным показом репозитория. Только отчитывается, не правит.
